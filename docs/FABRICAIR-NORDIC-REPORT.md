# FabricAir Nordic Circular Polyester Research Report

**Nordic Circular Construction (NCC) Project**
**Client:** FabricAir A/S
**Date:** November 2025

---

## Executive Summary

### The Opportunity

FabricAir can access **56,000-62,000 tonnes/year** of recycled polyester capacity across three Nordic countries by 2028-2029.

| Country | Partner | Capacity | Timeline |
|---------|---------|----------|----------|
| Finland | Rester | 6-12k t/yr | Now |
| Norway | NTG | 30k t/yr | ~2029 |
| Sweden | Rewin | 20k t/yr | Oct 2027 |

**Recommended Action:** Start with Rester (Finland) — they're operational today.

### Priority Contacts

| Partner | Country | Contact | Email |
|---------|---------|---------|-------|
| Rester | Finland | Jukka Pesola | hello@rester.fi |
| NTG | Norway | Bent Mathisen | btm@ntgas.no |
| Touchpoint | Finland | Noora Salonoja | noora.salonoja@touchpoint.fi |

---

## Project Background

### Mission

Map companies in Finland, Sweden, and Norway specializing in collection of used polyester fibers for recycling, with capacity to deliver high volume, high quality (mono) raw materials within FabricAir's quality standards.

### Material Requirements

| Parameter | Requirement |
|-----------|-------------|
| Material | 100% Polyester (PET) |
| Weight | ~300 g/m² |
| Color | White preferred |
| NOT suitable | Polyester/cotton blends (65/35) |

---

## Finland Analysis

### Market Overview

| Metric | Value |
|--------|-------|
| Textile waste | 85,770 t/year |
| Market size | €350-450M |
| Material recycling | <1% |
| EPR date | 2027-28 |

### Key Finding

**LSJH Turku recycling plant cancelled (April 2024).** Rester is now Finland's only large-scale textile recycler.

### Priority Partners (Finland) — Updated

| Rank | Organization | Type | Contact |
|------|--------------|------|---------|
| 1 | Rester | Recycler | hello@rester.fi |
| 2 | Puro | Healthcare | paavo.martikainen@puro.fi |
| 3 | Touchpoint | Aggregator | noora.salonoja@touchpoint.fi |
| 4 | Orion | Cleanroom | carolina.sved@orion.fi |

**Note:** ~~Lindström~~ downgraded to #9 — uses 65:35 poly-cotton blend (NOT compatible)

### Best 100% White Polyester Sources

| Sector | White % | 100% PET | Key Players |
|--------|---------|----------|-------------|
| Cleanroom/Pharma | >90% | Yes | Orion, Okmetic |
| Food Processing | 70-80% | Mixed | Valio, Fazer |
| Healthcare | ~60% | Often blends | HUS, Terveystalo |

---

## Sweden Analysis

### Market Overview

| Metric | Value |
|--------|-------|
| Textile waste | 60-80k t/year |
| Market size | €560M |
| Material recycling | <1% |
| EPR date | April 2028 |

### Key Finding

**Rewin project developing 20,000 t/year polyester recycling plant by October 2027.**

### Priority Partners (Sweden)

| Organization | Contact | Email |
|--------------|---------|-------|
| Rewin/Wargön | Joel Arnoldsson | via wargoninnovation.se |
| SIPTex/Sysav | Lars Carrick | lars.carrick@sysav.se |
| Elis Sweden | Patrik Nyholm | patrik.nyholm@elis.com |

### Best Source

Cleanroom sector (AstraZeneca, Pfizer) — 100% polyester, white, ~7,000 employees in Swedish cleanrooms.

---

## Norway Analysis

### Market Overview

| Metric | Value |
|--------|-------|
| Textile waste | ~100k t/year |
| Market size | €180-270M |
| Material recycling | <1% (97% exported) |
| EPR date | Under review |

### Key Finding

**NTG's patented defibration technology handles poly-cotton blends** — highly relevant for workwear waste. Scaling to 30,000 t/year by ~2029.

### Priority Partners (Norway)

| Organization | Contact | Email |
|--------------|---------|-------|
| NTG | Bent Mathisen | btm@ntgas.no |
| Nor Tekstil | Thomas Karlsen | thomas.karlsen@nortekstil.no |
| Fretex Miljø | Kristin Hareide | kristin.hareide@fretex.no |

---

## Nordic Comparison

| Factor | Finland | Sweden | Norway |
|--------|---------|--------|--------|
| **Recycler** | Rester | Rewin (2027) | NTG |
| **Capacity** | 6-12k t/yr | 20k t/yr | 30k t/yr |
| **Technology** | Mechanical | Chemical | Mech. defibration |
| **Unique** | Operational now | Automated sorting | Handles blends |

### Strategic Recommendation

1. **Immediate:** Partner with Rester (Finland)
2. **Medium-term:** Build NTG (Norway) relationship
3. **Long-term:** Multi-country supply chain (62k t/yr by 2029)

---

## Critical Findings (Phase 2 Updated)

| Finding | Impact |
|---------|--------|
| **Lindström uses 65:35 blends** | 400 t/year volume INCOMPATIBLE — downgraded to #9 |
| **Volume estimate revised** | 5-10k t/yr → **350-1,000 t/yr** accessible |
| LSJH Turku plant cancelled | Rester is now primary Finnish recycler |
| 65/35 blends NOT suitable | Target cleanroom/pharma for 100% PET |
| Rewin 20k t/yr by 2027 | Sweden domestic option coming |
| NTG handles poly-cotton | Opens access to workwear waste |
| Lindström owns Rester stake | Capacity may be locked — engage early |

---

## Recommended Actions

### This Week

1. Email Rester (hello@rester.fi) — Request meeting
2. Email NTG (btm@ntgas.no) — Explore partnership
3. Contact Touchpoint for feedstock discussions

### This Quarter

4. Request samples from Rester
5. Map cleanroom sector contacts (Orion, AstraZeneca)
6. Monitor Rewin development

### 2025 Roadmap

| Phase | Actions | Timeline |
|-------|---------|----------|
| Partner Engagement | Contact Rester, NTG, Touchpoint | Immediate |
| Technical Validation | Request samples, test compatibility | Q1 2025 |
| Commercial Terms | Negotiate pricing, volumes | Q1-Q2 2025 |
| Pilot Project | Small-scale production trial | Q2 2025 |
| Supply Agreement | Formalize partnership | H2 2025 |

---

## Risk Assessment

| Risk | Level | Mitigation |
|------|-------|------------|
| Rester capacity locked by Lindström | High | Engage immediately; explore NTG alternative |
| 65/35 blend contamination | High | Require certified 100% PET; target cleanroom |
| Quality variability | Medium | Rester pre-sorting; specify white only |
| rPET price premium (50-70%) | Medium | Volume commitments for better pricing |

---

## Contact Directory

### Recyclers

| Organization | Country | Contact | Email |
|--------------|---------|---------|-------|
| Rester | Finland | Jukka Pesola | hello@rester.fi |
| NTG | Norway | Bent Mathisen | btm@ntgas.no |
| Rewin/Wargön | Sweden | Joel Arnoldsson | via wargoninnovation.se |

### Feedstock Aggregators

| Organization | Country | Contact | Email |
|--------------|---------|---------|-------|
| Touchpoint | Finland | Noora Salonoja | noora.salonoja@touchpoint.fi |
| Puro | Finland | Paavo Martikainen | paavo.martikainen@puro.fi |
| Nor Tekstil | Norway | Thomas Karlsen | thomas.karlsen@nortekstil.no |

**Note:** ~~Lindström~~ excluded (65:35 blend NOT compatible)

### Project Contacts

| Contact | Role | Email |
|---------|------|-------|
| Darius Rinkevičius | FabricAir ESG | dri@fabricair.com |
| Jan Thomas Odegard | NCC Coordinator | janthomas@naturalstate.no |

---

## Delivery Status

| Assessment | Score | Notes |
|------------|-------|-------|
| Market Intelligence | 88% | Strong landscape mapping |
| Actionable Contacts | 50% | Strategic level, NOT operational |
| **Overall Delivery** | **75.5%** | Reconciled assessment |
| **Qualitative Grade** | **C+** | See QUALITATIVE-DELIVERY-ASSESSMENT.md |

**Critical Gap:** We provided CEO/executive contacts when the requirement asked for operational disposal contacts.

---

## Appendices

### A. Full Contact Directory

See accompanying Contact Directory document for complete list of 40+ verified contacts.

### B. Research Methodology

Research conducted using P1/P2/P3 verification protocol:
- P1: Critical claims verification
- P2: Operations and market data verification
- P3: Contact and strategic intelligence verification

### C. Source Documents

Research based on 30+ primary sources including:
- Government statistics (SYKE, Naturvårdsverket, SSB)
- Company websites and annual reports
- Industry association data
- Direct company verification

---

*Report prepared: November 2025*
*Project: Nordic Circular Construction (NCC)*
*Funded by: Nordic Innovation*
*Coordinator: Natural State AS*

---

**For questions contact:**
- FabricAir ESG: dri@fabricair.com
- NCC Project: janthomas@naturalstate.no
