# Q5: Initiatives — Sweden

> What public and private initiatives exist in Sweden for polyester collection and reuse?

---

## Key Finding

**Sweden has strong research infrastructure and the world's first automated textile sorting (SIPTex), but lacks domestic polyester recycling until Rewin launches (~2027).**

**⚠️ Critical Updates (Dec 2025):**
- **SIPTex being sold** — Sysav divesting due to lack of outlet markets
- **Rewin pilot permits secured** — Malmö facility moving to commissioning

---

## Initiative Overview

| Initiative | Type | Status | Impact |
|------------|------|--------|--------|
| **Rewin** | Private | Development | Game-changer (20k t/yr) |
| **SIPTex** | Public-Private | Operational | World-first sorting |
| **RE:Source** | Public | Operational | Major funding |
| **Textile 2030** | Industry | Operational | Roadmap |

---

## Major Initiatives

### Rewin Project (CRITICAL)

| Attribute | Detail |
|-----------|--------|
| Lead | Wargön Innovation |
| Technology | Chemical fiber-to-fiber recycling (catalytic glycolysis) |
| Full-scale capacity | 20,000 tonnes/year |
| Pilot capacity | ~20 tonnes/year |
| Project timeline | Oct 2023 – Oct 2027 |
| Funding | Swedish Energy Agency |
| Full-scale location | Vargön (near Gothenburg, industrial symbiosis with Vargön Alloys) |
| **Pilot location** | **Sege Industrial Park, Malmö** |

**Status (Dec 2025):**
- ✅ **Pilot plant permits secured** (building + environmental)
- ✅ Commissioning targeted autumn 2025, operations before end 2025
- ⚠️ Full-scale "Oct 2027" is project end date, not confirmed launch

**Feedstock Requirements:**
- **≥80% polyester content**
- No requirements for color or print
- Accepts buttons/zippers (no pre-removal needed)
- Sources: textile sorters, industrial production waste
- **100% polyester is acceptable** (not limited to blends)

**Output:** Virgin-quality polyester granulate, suitable for new production

**Partners:**
- Gina Tricot, KappAhl (fashion retail)
- Helly Hansen (outdoor/workwear)
- Dual Borgstena, ScandCycle, National

**Contact:** Joel Arnoldsson — Innovation Manager, Wargön Innovation

---

### SIPTex — ⚠️ BEING DIVESTED

| Attribute | Detail |
|-----------|--------|
| Operator | Sysav (Malmö) |
| Technology | NIR + visual spectroscopy |
| Capacity | 24,000 tonnes/year (~30% of Swedish textile waste) |
| Status | ⚠️ **Sysav divesting** |
| Achievement | World's first automated textile sorting |

**⚠️ Critical Update (Dec 2025):**

Sysav announced it is **starting a process to sell/divest SIPTex:**
- Investment assumed domestic offtake markets (2019 decision)
- **Since 2024:** No stable outlet for sorted fractions
- **~1,200 tonnes unsorted textiles** sitting at facility at decision time
- Sysav does NOT expect market improvement during 2026-2027
- Textile EPR (2028) will change roles/responsibilities

**How it works:**
1. Textiles fed onto conveyor
2. NIR identifies fiber composition
3. Air jets separate by type
4. >90% purity for polyester fraction

**Implication:** SIPTex's future role depends on who acquires it and whether recyclers/PROs can contract stable offtake chains.

**Contact:** Lars Carrick — lars.carrick@sysav.se

---

### RE:Source

| Attribute | Detail |
|-----------|--------|
| Type | Strategic Innovation Program |
| Funding | ~SEK 50 million/year |
| Focus | Resource efficiency, circular economy |
| Projects | ~300 since inception |

**Textile Projects:**
- Fiber recycling technology
- Sorting innovation
- Design for circularity
- Business model development

**Contact:** Cecilia Tall — cecilia.tall@ri.se (RISE)

---

### Textile 2030

| Attribute | Detail |
|-----------|--------|
| Type | Industry initiative |
| Lead | TEKO, Sustainable Fashion Academy |
| Focus | Sustainability roadmap |
| Members | Major brands and retailers |

**Goals:**
- Climate neutral by 2030
- Circular business models
- Sustainable materials
- Transparency

---

## Research Initiatives

### Chalmers University

| Focus | Lead Researcher |
|-------|-----------------|
| Chemical recycling | Christian Ekberg |
| Polymer chemistry | Multiple |
| Separation technology | Multiple |

**Contact:** Christian Ekberg — che@chalmers.se

### RISE (Research Institutes of Sweden)

| Focus | Status |
|-------|--------|
| Textile innovation | Active |
| Circular economy | Active |
| Materials testing | Active |

### Borås University

| Focus | Status |
|-------|--------|
| Textile technology | Active |
| Smart textiles | Active |
| Sustainable design | Active |

---

## Collection Initiatives

### Human Bridge

| Attribute | Detail |
|-----------|--------|
| Coverage | 118 municipalities |
| Model | Charity/social enterprise |
| Focus | Collection for export/resale |

### Myrorna

| Attribute | Detail |
|-----------|--------|
| Type | Second-hand retail |
| Coverage | Nationwide |
| Model | Donation and resale |

---

## Company Initiatives

### Elis Cleanroom — Nordic Aggregator (NEW)

| Attribute | Detail |
|-----------|--------|
| Type | Cleanroom textile services |
| Nyköping plant | **Only Swedish facility for GMP Grade A/B garments** |
| Processing | ~5,000 kg/day validated wash/dry/sterilize |
| EOL routing | **Alsico/ARX recycling partnership** |
| Contact | cleanroom@elis.com |

**⚠️ Cleanroom Material Reality (Dec 2025):**
- Typically **99% PET + 1% carbon fiber** (not pure 100% PET)
- Colors: Mixed (white not exclusive in cleanrooms)
- Lindström 2024: Cleanroom textiles 100% to recycling

**Strategic Significance:** Elis operates multi-client cleanroom services across SE/NO/DK/FI — can aggregate EOL textiles from multiple pharma/tech companies.

### Fristads REACT

| Attribute | Detail |
|-----------|--------|
| Type | Take-back program |
| Status | Fully launched 2024 |
| Focus | Workwear recycling |
| Goal | Closed-loop production |

### H&M Garment Collecting

| Attribute | Detail |
|-----------|--------|
| Type | In-store collection |
| Coverage | All stores globally |
| Volume | Major but consumer-focused |

---

## Government Support

### Vinnova

| Type | Support |
|------|---------|
| Innovation funding | Major programs |
| Relevant calls | Circular economy, materials |

### Swedish Energy Agency

| Type | Support |
|------|---------|
| Energy transition | Funding for Rewin |
| Industrial transformation | Supporting chemical recycling |

### Business Sweden

| Type | Support |
|------|---------|
| Export promotion | Supporting Swedish cleantech |
| Market access | International connections |

---

## Funding Landscape

| Program | Focus | Typical Grant |
|---------|-------|---------------|
| RE:Source | Circular economy | SEK 1-10M |
| Vinnova | Innovation | SEK 2-20M |
| Energy Agency | Clean tech | SEK 5-50M |
| EU Horizon | Research | €1-5M |

---

## Strategic Recommendations

1. **Engage Rewin NOW** — Key future partner (~2027), pilot operational soon
2. **Monitor SIPTex sale** — Sorting infrastructure uncertain, watch who acquires
3. **Monitor RE:Source** — Funding opportunities
4. **Connect via Chalmers** — Technical credibility

---

## Priority Contacts

| Initiative | Contact | Email |
|------------|---------|-------|
| **Rewin** | Joel Arnoldsson | via wargoninnovation.se |
| **SIPTex** | Lars Carrick | lars.carrick@sysav.se |
| **RE:Source** | Cecilia Tall | cecilia.tall@ri.se |
| **Chalmers** | Christian Ekberg | che@chalmers.se |
| **Elis Cleanroom** | Global | cleanroom@elis.com |
| **Elis Sweden** | Ulrika Viderum (Sustainability) | via se.elis.com |

---

*Last updated: 2025-12-16*
