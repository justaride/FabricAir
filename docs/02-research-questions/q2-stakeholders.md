# Q2: Value Chain Stakeholders

> Who are the main stakeholders in production, use, and reuse of polyester textiles in Finland?

**Status:** Verified

---

## Summary

Finland's textile ecosystem includes 200+ companies with mandatory collection since January 2023 (2 years ahead of EU).

---

## Value Chain Overview

```
Production → Use → Collection → Sorting → Recycling → New Products
```

---

## 1. Production

### Fiber & Material Producers

| Company | Role |
|---------|------|
| Neste | Neste RE™ for renewable polyester |
| Intermedius | Recycled PET felt |
| VIP-Fil | Polyester yarns |

### Workwear Manufacturers

| Company | Focus |
|---------|-------|
| Image Wear | Uniforms, EU Digital Product Passport pilot |
| Touchpoint | Sustainable workwear, Rester majority owner |
| Marimekko | Fabric production, Rester partner |

---

## 2. Use

### Textile Service Companies

| Company | Market Share | Key Detail | Compatibility |
|---------|--------------|------------|---------------|
| **Puro** | ~50% population | Healthcare focus | ✅ Priority |
| **SOL** | — | 10,000+ employees | Verify material |
| **Elis** | — | Pan-European, RABC-compliant | Verify material |
| ~~**Lindström**~~ | 50-70% | 17M kg textiles | ⚠️ **NOT COMPATIBLE** |

**Critical Finding (Phase 2):** Lindström uses 65:35 poly-cotton blends — NOT suitable for FabricAir. Downgraded from priority partner.

### Major Users

| Sector | Examples |
|--------|----------|
| Healthcare | HUS, Terveystalo, Mehiläinen |
| Food | Valio, Fazer, Atria |
| Industrial | KONE, Wärtsilä, YIT |

---

## 3. Collection

### Municipal System

- 31 waste management areas
- 97% population coverage
- 25 municipal waste companies
- Mandatory collection since Jan 2023

### Key Organizations

| Organization | Coverage |
|--------------|----------|
| LSJH | 450,000 inhabitants, 18 municipalities |
| HSY | 1+ million residents, 11 collection points |
| UFF | 12M kg/year, 95.5% reuse rate |

---

## 4. Sorting

| Facility | Status |
|----------|--------|
| LSJH Turku | ⚠️ **Outsourcing to Turun Ekotori (2026)** |
| Turun Ekotori | Taking over sorting operations 2026 |

**⚠️ LSJH Update (Dec 2025):** LSJH announced outsourcing textile sorting/QA to Turun Ekotori starting early 2026. 20k t/yr refinement plant abandoned. Lemppari store closing end 2025.

---

## 5. Recycling

| Company | Capacity | Technology |
|---------|----------|------------|
| **Rester** | 6,000-12,000 t/yr | Mechanical (primary recycler) |
| IFC | 30,000 t/yr (2025) | Chemical (cotton-rich) |

**Critical:** With LSJH plant cancelled, Rester is Finland's only large-scale textile recycler.

---

## 6. Research

| Institution | Focus |
|-------------|-------|
| VTT | PESCO-UP (€48M polyester recycling) |
| Aalto University | Ioncell®, chemical recycling |
| Telaketju | 17+ company network |

---

## Key Partnerships

| Partnership | Nature |
|-------------|--------|
| Lindström → Rester | **2nd largest shareholder**, ~500 t/year to Rester |
| Touchpoint → Rester | Majority ownership |
| Elis → Rester | Pilot recycling |
| K Group → Touchpoint → Rester | Workwear take-back |

**Lindström-Rester Relationship (Dec 2025):**
- Lindström became shareholder via Mar 2022 investment round
- Sends ~500 t/year end-of-life textiles to Rester
- **NO exclusive capacity access confirmed** in public sources
- 70% recycling achieved 2024 (target: 100% by 2025)

---

## Priority Contacts (Updated)

| Organization | Contact | Email | Status |
|--------------|---------|-------|--------|
| Rester | Jukka Pesola | hello@rester.fi | ✅ Priority |
| Puro | Paavo Martikainen | paavo.martikainen@puro.fi | ✅ Priority |
| Touchpoint | Noora Salonoja | noora.salonoja@touchpoint.fi | ✅ Priority |
| Orion | Carolina Sved | carolina.sved@orion.fi | ✅ 100% PET |

**Note:** ~~Lindström~~ removed — 65:35 blend NOT compatible

---

*Last updated: 2025-12-16*
